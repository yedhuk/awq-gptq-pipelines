...
Current focus: **FP16 vs INT4 AWQ** on `meta-llama/Llama-3.2-3B-Instruct`. The benchmarking engine is designed to be reusable for future quantization methods (GPTQ, GGUF, FP8, SmoothQuant, ...) by swapping only the model-loading step.
...
│   ├── llama_3_2_3b_base_awq_inference.ipynb   # Benchmark engine: loads FP16 + AWQ, runs prompts, logs metrics
...
├── pipelines/                                  # Reserved for extracted/reusable pipeline code
├── awq-requirements.txt                        # Python dependencies
...
1. **Quantize the base model** — run `notebooks/llama_3_2_3b_awq_experiments.ipynb` to produce a local AWQ checkpoint under `notebooks/models/`.
2. **Benchmark** — run `notebooks/llama_3_2_3b_base_awq_inference.ipynb` to load both the FP16 baseline and the quantized checkpoint, run them over a fixed prompt set (QA, reasoning, coding, summarization, long-context, math), and collect metrics.
...
- Additional quantization methods: GPTQ, SmoothQuant, LLM.int8(), FP8, GGUF, HQQ, AQLM, SpQR
